...
import { faCircleInfo, faUser, faRightFromBracket, faGlobe} from '@fortawesome/free-solid-svg-icons';
...
export default function Header({ onNavClick, auth, setAuth, onLogout }) {
  ...
  return (
    <header className="bg-gradient-to-r from-red-500 via-yellow-500 to-blue-500 text-white py-1 px-2 w-full shadow-md fixed top-0 z-50 static animate-rainbow">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-2 md:gap-4">
          <div className="flex items-center gap-2 md:gap-4">
            <div className="w-12 h-12 md:w-24 md:h-24 flex items-center justify-center">
              <button onClick={() => onNavClick('home')}
                title="Home"
                aria-label="Home">
                <FontAwesomeIcon icon={faGlobe} className="text-2xl md:text-4xl" />
              </button>
            </div>
            <div className="flex flex-col items-start">
              <h1 className="text-lg md:text-2xl font-bold">Paris olympics 2024</h1>
              <p className="text-sm md:text-lg">Athlete Data</p>
            </div>
          </div>
          ...
          <nav className="flex flex-row gap-1 md:gap-4">
            {auth ? (
              <>
                <button
                  onClick={() => onNavClick('olympics2024')}
                ...
                >
                  <FontAwesomeIcon icon={faCircleInfo} className="mr-1 md:mr-2" />
                  <span className="hidden md:inline">Olympic</span>
                </button>
